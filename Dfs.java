		...
		if(it != null){
			while(it.hasNext()){
				...
				if(pomOdjazd.getCzas().czyNieMniejszy(poczatekCzasu)){
					dfs(new ElementStosu(pomOdjazd.getCzas(), przystanekStartowy, pomOdjazd.getKurs().getLinia(), pomOdjazd, false));
				}
			}
		}
		return this.wynik;
	...
	private void dfs(ElementStosu akt){
		...
		if(akt.czyWysiadam() && !(akt.getPrzystanek()==przystanekDocelowy)){
			...
			while(it1.hasNext()){
				...
				if(tempOdjazd.getCzas().czyNieMniejszy1(akt.getCzas()) && !this.czyWystepujeLinia(tempOdjazd.getKurs().getLinia())){
					ElementStosu tempElt = new ElementStosu(tempOdjazd.getCzas(), akt.getPrzystanek(), tempOdjazd.getKurs().getLinia(), tempOdjazd, false);
					dfs(tempElt);
				}
			}
			/*
		...
		if(nast != null){
			if(!this.czyWystepujePrzystanek(nast.getPrzystanek())){
				nastElt = new ElementStosu(nast.getCzas(), nast.getPrzystanek(), nast.getKurs().getLinia(), nast, true);
				dfs(nastElt);
			}
		}
		historia.pop();
